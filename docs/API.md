...
| `BackgroudGif` | string | URL to a `.gif` the device fetches as background — required with `NewFlag: 1`; omit with `NewFlag: 0` ✅. **Must be `.gif`** ✅ — a `.jpg`/`.png` URL returns `error_code 0` (accepted) but the panel gets stuck showing no background/loading; only tested with `.gif` did the background actually render. **GIF palette index 0 is treated as transparent** ✅ — whatever colour sits at palette index 0 vanishes (a solid icon then shows as just its antialiased outline). Reserve index 0 for an **unused sentinel** (put real colours in indices 1+, save with `optimize=False` so it isn't collapsed) — then no pixel is transparent and any background colour renders. (A full 256-entry palette renders fine — an earlier "256 crashes the device" note was wrong; the real crash cause was an item `color` sent as a JSON list instead of `"#RRGGBB"`. Minimal palettes are still preferred for smaller GIFs.) |
| `ItemList` | array | list of item objects (below). **Must not be empty** ✅ — sending `"ItemList": []` (with or without a `BackgroudGif`) crashes the panel; it went unresponsive and rebooted (reported 2026-08-24 on a card that draws every figure as artwork and so had nothing to poll). A page with no items has no item list to send: push the artwork with `Draw/SendHttpGif` instead. |
