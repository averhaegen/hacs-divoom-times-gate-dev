...
async def test_options_flow_switches_preset(hass, mock_config_entry) -> None:
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        {"active_preset": "energy", "preset_action": "switch"},
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"next_step_id": "save"}
    )
...
async def _open_preset_step(hass, entry):
...
async def test_switching_preset_keeps_the_edits_to_the_previous_one(
    hass, mock_config_entry
) -> None:
    """Edits made before switching stay under the preset they were made in."""
    mock_config_entry.add_to_hass(hass)
    hass.config_entries.async_update_entry(
        mock_config_entry,
        options={
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"screens": {"page_type": "image"}}
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"next_step_id": "preset"}
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"active_preset": "energy", "preset_action": "switch"}
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"next_step_id": "save"}
    )

    assert result["data"]["presets"]["default"][0] == {"page_type": "image"}
    assert result["data"]["active_preset"] == "energy"


@pytest.mark.parametrize(
    ("action", "expected"),
    [("create", [{"page_type": "off"}] * 5), ("copy", [{"page_type": "clock"}] * 5)],
)
async def test_preset_step_adds_a_preset(
    hass, mock_config_entry, action, expected
) -> None:
    mock_config_entry.add_to_hass(hass)
    hass.config_entries.async_update_entry(
        mock_config_entry,
        options={
            "presets": {"night": [{"page_type": "clock"}] * 5},
            "active_preset": "night",
        },
    )

    result = await _open_preset_step(hass, mock_config_entry)
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        {"active_preset": "night", "preset_action": action, "preset_name": "new"},
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"next_step_id": "save"}
    )

    assert result["data"]["active_preset"] == "new"
    assert result["data"]["presets"]["new"] == expected
    assert "night" in result["data"]["presets"]


async def test_preset_step_renames_and_deletes(hass, mock_config_entry) -> None:
    mock_config_entry.add_to_hass(hass)
    hass.config_entries.async_update_entry(
        mock_config_entry,
        options={
            "presets": {
                "night": [{"page_type": "clock"}] * 5,
                "day": [{"page_type": "off"}] * 5,
            },
            "active_preset": "night",
        },
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        {"active_preset": "night", "preset_action": "rename", "preset_name": "evening"},
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"next_step_id": "preset"}
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"active_preset": "day", "preset_action": "delete"}
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"next_step_id": "save"}
    )

    assert sorted(result["data"]["presets"]) == ["evening"]
    assert result["data"]["active_preset"] == "evening"


@pytest.mark.parametrize(
    ("payload", "error"),
    [
        ({"preset_action": "create", "preset_name": ""}, "preset_name_required"),
        ({"preset_action": "copy", "preset_name": "night"}, "preset_name_taken"),
        ({"preset_action": "delete"}, "preset_last"),
    ],
)
async def test_preset_step_refuses_bad_input(
    hass, mock_config_entry, payload, error
) -> None:
    mock_config_entry.add_to_hass(hass)
    hass.config_entries.async_update_entry(
        mock_config_entry,
    ...
    result = await _open_preset_step(hass, mock_config_entry)
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"active_preset": "night", **payload}
    )
...
async def test_energy_step_writes_to_the_preset_you_name(
    hass, mock_config_entry
) -> None:
    """Generating screens leaves the preset you were editing alone."""
    mock_config_entry.add_to_hass(hass)
    hass.config_entries.async_update_entry(
        mock_config_entry,
        options={
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"preset_name": "power"}
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"next_step_id": "save"}
    )

    assert result["data"]["active_preset"] == "power"
    assert result["data"]["presets"]["night"] == [{"page_type": "clock"}] * 5
    assert len(result["data"]["presets"]["power"]) == 5
