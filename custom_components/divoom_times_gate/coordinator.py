...
class TimesGateCoordinator(DataUpdateCoordinator[dict[int | str, str]]):
            ...
            items.append(
                {
                    ...
                    "dir": 0,
                    "font": int(sensor.get("font", page.get("font", DEFAULT_DEVICE_FONT))),
    ...
    # Native device-rendered element types — moved to const.NATIVE_KIND_TYPES
    # (shared with card headers); kept as a class alias for existing callers.
    _NATIVE_KIND_TYPES = NATIVE_KIND_TYPES
    ...
    async def _build_dispdata_items(
        ...
        for row, raw in enumerate(raw_items):
            ...
            common = {
                ...
                "dir": 0,
                "font": int(raw.get("font", page.get("font", DEFAULT_DEVICE_FONT))),
